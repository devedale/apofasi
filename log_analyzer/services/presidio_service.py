...
class PresidioService:
    ...
    def _get_operators(self) -> Dict[str, OperatorConfig]:
        ...
        for entity_name, strategy_name in strategies.items():
            params = strategy_configs.get(strategy_name, {}).copy()

            # Normalize and backfill parameters for specific strategies
            # WHY: Presidio's mask operator requires `masking_char`. Some configs
            # may provide `mask_char` or omit optional fields. We normalize here
            # to avoid runtime validation errors.
            if strategy_name == "mask":
                if "mask_char" in params and "masking_char" not in params:
                    params["masking_char"] = params.pop("mask_char")

                if "masking_char" not in params:
                    params["masking_char"] = "*"
                if "chars_to_mask" not in params:
                    params["chars_to_mask"] = 0  # 0 => mask all characters
        ...
        for rec_conf in ad_hoc_recognizers:
            ...
            if entity_name and strategy_name:
                ...
                # Apply same normalization for mask strategy here as well
                if strategy_name == "mask":
                    if "mask_char" in params and "masking_char" not in params:
                        params["masking_char"] = params.pop("mask_char")
                    if "masking_char" not in params:
